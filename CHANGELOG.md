...
- 🧱 修复「daed 看起来丢了整份配置」这一类故障：dae-core 的 pin 停在 2026-04-22，而 dae 的配置面在 2026-09-16 的 kdae 重构里长出了 `disable_thp`、`auto_sniff_punt`、`bpf_conn_state_map_size`（dns 段还有 `optimistic_stale_reply_ttl`）。daed 用这个内嵌核解析自己存下来的 global/dns/routing 段，而解析器拒绝未知键——**一个键就让整份配置解析失败**，`restoreRunningState()` 随即把 `systems.running` 置 0、数据面留在空配置：日志里只有 `Routing match set len: 1/1024` 与 `Health check will only verify 1 outbounds ... [direct]`，全部路由规则/节点组/DNS 策略静默失效，面板上仍显示配置为选中状态。2026-09-20 实机就是这样：`global` 段里那行 `disable_thp:"true"` 抄自 dae 自带的 `/etc/dae/example.dae`，daed 报 `Failed to restore last running state: failed to parse "global": unexpected key: disable_thp`。新增 [.github/patches/dae-core-config-compat.patch](.github/patches/dae-core-config-compat.patch)，按上游定义把四个键补进内嵌核（`disable_thp`、`auto_sniff_punt`、`bpf_conn_state_map_size` 在 daed 后端本就是无实现项——wing 不跑 `cmd/` 的 THP 控制、4 月版没有 `InferSniffPunt`、连接跟踪表大小是编译期常量；`optimistic_stale_reply_ttl` 同理），并在字段注释与 daed 面板读取的 desc 里写明"仅为解析兼容"，避免被当成真实开关。`pin-daed-core.py` 的补丁支持改为**有序补丁序列**：逐份在临时副本上真实应用（不再是各份独立 dry-run，顺序依赖因此可测）、认识该选项的内核自动免打、不可应用则十分钟内失败；`diy-part2-daed.sh` 另装入这份补丁。
...
- 🧱 修复 daed 面板「流量统计不显示流量」：wing（dae-wing@dc503088）从**包级** `control.SnapshotRuntimeStats` 读 `uploadRate`/`uploadTotal`，而固定核（4 月）的数据面只把字节记进每个控制面实例自己的 `runtimeStats`（`tcp.go` 的 `RelayTCPContextWithRecords(..., c.runtimeDownloadRecorder(), c.runtimeUploadRecorder())`，`udp.go`/`dns_listener.go` 同理），`newControlPlaneWithContextOptions` 又总是给实例装上自己的 store，包级那份历史因此永远是空的——`runtimeStatsStore()` 只在实例没有 store 时才回落到全局，而生产路径上永远不会。2026-09-20 实机面板正是如此：`activeConnections=28, udpSessions=67` 而 `uploadTotal`/`downloadTotal` 全 0（同一时刻宿主侧 `dae0` 已累计 12/46 MiB，证明流量一直在过）。新增 [.github/patches/dae-core-traffic-stats.patch](.github/patches/dae-core-traffic-stats.patch)：按实例记账的同时把每一笔镜像进包级历史（仅当 `runtimeStatsStore() != globalRuntimeStats` 才另记一次，保证一字节只记一次），并加一条 `control` 回归测试；该测试在未打补丁的固定核上报 `package snapshot totals = (0, 0), want (1200, 800)`（与面板症状同源），打完后同时断言「面板口径有数」与「每字节只记一次」。既有的 runtime-stats 测试没抓到这个错配，是因为它们的夹具用的是没有实例 store 的 plane，记录器会回落到全局口径。
- 🧱 `pin-daed-core.py` 的补丁序列扩到三份，探针也从「单文件若干符号」改为「多组 *文件+符号* 备选」：任一组命中即视为内核已自行解决（例如 dae main 把 TCP 中继直接记进包级口径；注意不能用「两个全局记录器同时出现」当判据——未修的 4 月核里那个没人调用的 `RelayTCPContext` 包装函数同样含这两个名字，会误判为已修），否则逐份在临时副本上真实应用。实测三棵树判定正确：固定核=三份都需打、打完后=三份都跳过、dae main=`response_ttl` 需打而配置兼容/流量统计跳过。`diy-part2-daed.sh` 同步装入第三份补丁，`check-dae-config-compat.py` 的 `DAE_CORE_PATCHES` 重放随之覆盖三份（验证输出：`series applied: dae-core-response-ttl.patch, dae-core-config-compat.patch, dae-core-traffic-stats.patch, 56 config keys`、`all 36 template keys are accepted`）。
